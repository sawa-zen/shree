 ...
 * VBOを生成する
 */
export const createVbo = (gl: WebGLRenderingContext, verticies: number[]): WebGLBuffer => {
  // バッファオブジェクトの生成
 ...
 * IBOを生成する
 */
export const createIbo = (gl: WebGLRenderingContext, index: number[]): WebGLBuffer => {
  // バッファオブジェクトの生成
 ...
 * ブレンディングの切り替え
 */
export const switchBlending = (gl: WebGLRenderingContext, transparent: boolean) => {
  if (transparent) {
    // ブレンディングを有効
    gl.enable(gl.BLEND);
    gl.blendFunc(
